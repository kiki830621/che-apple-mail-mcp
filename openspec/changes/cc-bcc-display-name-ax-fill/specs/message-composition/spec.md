...
`update_draft` SHALL gate its delete of the old draft on this receipt: when the replacement's receipt reports a definitive mismatch, the old draft SHALL be kept and the result SHALL say `deleted_old: false` with a note naming the mismatch; an unavailable or not-found receipt SHALL NOT gate the delete.
