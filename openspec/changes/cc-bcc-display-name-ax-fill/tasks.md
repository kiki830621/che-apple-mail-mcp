...
- [x] 5.2 [P] 三份 rules 第 6 類同步：`.claude/rules/compose-wrapper-free.md`、`plugin/rules/compose-wrapper-free.md`、`~/Developer/che-claude-config/rules/common-mail-compose.md` — 第 6 類改為「send 帶顯示名收件人」，並註明 draft 的 Bcc 會被切出且不還原。驗證：三份檔案的第 6 類列文字 diff 一致（逐字相同段落）。
...
- [x] 6.2 Live gate（attended，2026-09-07 執行）：對真實帳號 `create_draft` 帶 named Cc + named Bcc（初始 Bcc 隱藏），Mail 草稿顯示人名 token，result 含 `recipients_verified: true` 與 `bcc_field_revealed: true`；再以 `update_draft` 重跑一次；最後刪除測試草稿。驗證：指令與觀測值貼進 #404 closing comment；未跑 → 貼 `blocked-on-setup` 保持 open 並在 description 加 caveat（`.claude/rules/deferred-live-verification.md`）。
  ...
  - R2-6 執行結果（2026-09-08 01:05 +08:00，head `59f021e`，release build，螢幕解鎖後 attended）：起始狀態 Bcc 欄位隱藏（以拋棄式視窗切回）；`create_draft` named To／Cc（含一個裸位址）／Bcc + `from_address` → 結果含 `bcc_field_revealed: true`、sender verified；**`recipients_verified: false — recipients_receipt: unavailable`**（in-process 收據掃描逾 45 s 被放棄，即 #406 已揭露的降級態；非「not found」、草稿保留）。獨立 AppleScript 讀回：to `收件人甲 <to-gate@example.org>`、cc `副本乙 <cc-gate@example.org>` + 裸位址 `cc2-gate@example.org`、bcc `密件丙 <bcc-gate@example.org>`，名稱與位址全數與請求一致；測試草稿已刪。判定：#404 的填入與 Bcc 揭露在最終產生器上 **PASS**；收據三態揭露如 description 所述運作，但 `recipients_verified: true` 本輪未觀測到（R1 2026-09-07 曾觀測到）。差異最可能來自 #406 的掃描時間（delta `48768e7..9013cd5` 未動 receipt 腳本與逾時；同一收據腳本以 osascript 獨立執行僅 3.3 s），但這不是受控比較：本 PR 的 R1 修正 `67f04b2` 已把收據取回從三次重試改為首次失敗即停，R1 的 `true` 是在重試版上觀測到的——歸因有證據支持、未被證明（PR #407 R3 DA-1）。本次成功 run 結構上碰不到第四項構造（cleanup 同標題唯一性／WINDOWLEFTOPEN 只在錯誤路徑執行），該項維持 `ComposeCleanupSheetTests` 單元覆蓋。
